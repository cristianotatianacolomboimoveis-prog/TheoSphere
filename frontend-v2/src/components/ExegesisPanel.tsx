...
export default function ExegesisPanel({
  verse,
  onClose,
  hideHeader = false,
}: {
  ...
  useEffect(() => {
    const fetchExegesis = async () => {
      setLoading(true);
      const isNT =
        /\b(Mateus|Marcos|Lucas|João|Atos|Romanos|Coríntios|Gálatas|Efésios|Filipenses|Colossenses|Tessalonicenses|Timóteo|Tito|Filemom|Hebreus|Tiago|Pedro|Judas|Apocalipse)\b/i.test(
          verse,
        );
      ...
      try {
        ...
        if (typeof content === "object") {
          ...
          const jsonMatch = (content as string).match(
            /```(?:json)?\s*([\s\S]*?)\s*```/,
          ) || [null, content as string];
          try {
            setData(JSON.parse(jsonMatch[1]!) as ExegesisData);
          } catch {
            setMarkdownFallback(content as string);
          }
        }
      } catch (err) {
        console.error(err);
                ...
                {(realInterlinear ?? data?.interlinear ?? []).map((w, i) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-[9px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-900/30">
                        {w.strong}
                      </span>
                    </div>
                    <p className="text-xs font-serif italic text-blue-600 mb-1">
                      {w.transliteration}
                    </p>
                    <p className="text-[10px] text-gray-400 dark:text-white/20 mb-3 font-mono">
                      {w.morphology}
                    </p>
                    <div className="mt-auto pt-2 border-t border-gray-100 dark:border-white/5">
                      <p className="text-sm font-serif font-bold text-gray-700 dark:text-gray-200">
                        "{w.translation}"
                      </p>
                    </div>
                  </div>
            ...
            >
              <div className="space-y-6">
                {data?.lexical_analysis.map((l, i) => (
                  ...
                  >
                    <h4 className="text-lg font-serif font-bold text-gray-800 dark:text-white mb-2">
                      {l.word}
                    </h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400 font-serif leading-relaxed mb-3 text-justify">
                      {l.bdag_halot_sense}
                    </p>
                    <p className="text-[11px] text-blue-600/60 italic font-medium">
                      {l.academic_discussion}
                    </p>
                  </div>
                ))}
                <div className="p-5 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20 italic">
                  <p className="text-sm text-gray-500 dark:text-white/40 font-serif leading-relaxed">
                    "{data?.syntactic_notes}"
                  </p>
                </div>
              </div>
            </StudySection>
            ...
            <StudySection title="Comentários Técnicos" icon={Info}>
              <div className="space-y-4">
                {data?.technical_commentary.map((c, i) => (
                  ...
                  >
                    <span className="text-[10px] font-black text-blue-600 uppercase mb-3 block tracking-widest">
                      {c.source}
                    </span>
                    <p className="text-[13px] font-serif italic text-gray-600 dark:text-gray-300 leading-relaxed">
                      "{c.view}"
                    </p>
                  </div>
                ))}
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center gap-3">
          <Icon
            className={`w-4 h-4 ${isOpen ? "text-blue-600" : "text-gray-300 group-hover:text-gray-400"}`}
          />
          <span
            className={`text-[11px] font-bold uppercase tracking-[0.2em] transition-all ${isOpen ? "text-gray-900 dark:text-white" : "text-gray-400 group-hover:text-gray-500"}`}
          >
            {title}
          </span>
        </div>
