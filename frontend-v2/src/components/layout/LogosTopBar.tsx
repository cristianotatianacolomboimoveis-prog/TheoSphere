...
export function LogosTopBar() {
  ...
  const handleCommand = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const input = (e.currentTarget.elements.namedItem("command") as HTMLInputElement)?.value?.trim();
    if (!input) return;

    // Simple command routing
    const lower = input.toLowerCase();
    if (lower.startsWith("gen") || lower.startsWith("gên") || /^[1-3]?\s?[a-z]/i.test(lower)) {
  ...
  return (
    <div className="h-10 bg-[#E8EBF0] dark:bg-[#1E252B] border-b border-gray-300 dark:border-black/20 flex items-center px-4 gap-4 z-[60]">
      ...
      <form onSubmit={handleCommand} className="flex-grow max-w-2xl relative group">
        ...
        />
        <button type="submit" className="absolute right-1 top-1/2 -translate-y-1/2 px-2 py-0.5 bg-gray-100 dark:bg-white/5 text-[10px] font-bold text-gray-500 rounded border border-gray-200 dark:border-white/10 hover:bg-blue-600 hover:text-white hover:border-blue-600 transition-colors">
            IR
        </button>
      </form>
      ...
      <div className="flex items-center gap-1">
        <TopBarButton icon={Library} label="Biblioteca" onClick={() => setActiveTool("library")} />
        <TopBarButton icon={Search} label="Busca" onClick={() => setActiveTool("exegesis")} />
        <TopBarButton icon={Sparkles} label="Factbook" onClick={() => setActiveTool("factbook")} />
        <div className="w-px h-4 bg-gray-300 dark:bg-white/10 mx-2" />
        <TopBarButton icon={Layout} label="Layouts" onClick={() => setActiveTool("exegesis")} />
      </div>
...
function TopBarButton({ icon: Icon, label, onClick }: { icon: any, label: string, onClick?: () => void }) {
  return (
    <button 
      onClick={onClick}
