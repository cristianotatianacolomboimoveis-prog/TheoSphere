...
export default function WordStudy({
  ...
  const fetchLibraryExcerpts = async (entry: StrongsEntry) => {
    ...
    try {
      const qs = new URLSearchParams({
        term: entry.lemma || entry.transliteration,
  ...
  const results = useMemo(() => {
    ...
    return language === "greek"
      ? searchStrongs(query)
      : searchStrongsHebrew(query);
  }, [query, language]);

  const allEntries = useMemo(() => {
    return language === "greek"
      ? Object.values(STRONGS_GREEK)
      : Object.values(STRONGS_HEBREW);
  }, [language]);
    ...
    <div className="flex flex-col h-full bg-[#F3F4F6] dark:bg-[#0A0D14] text-gray-900 dark:text-gray-100 overflow-hidden border-l border-gray-200 dark:border-white/10 shadow-2xl">
        ...
        {!hideHeader && (
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-serif font-bold text-gray-900 dark:text-white leading-tight">
                  Estudo de Palavras
                </h2>
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">
                  Dicionário Léxico & Concordância
                </p>
              </div>
            </div>
            <button
          ...
          {selectedEntry ? (
            ...
            >
              ...
              <div className="text-center border-b border-gray-200 dark:border-white/10 pb-8">
                <p
                  className="text-6xl font-serif font-bold text-gray-900 dark:text-white mb-4"
                  dir="auto"
                >
                  {selectedEntry.lemma}
                </p>
                <p className="text-xl text-indigo-600 font-mono font-bold italic">
                ...
                <div className="flex items-center justify-center gap-3 mt-6">
                  ...
                  <span className="px-3 py-1 rounded-md bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/20 text-[10px] font-black text-indigo-600">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-[#0D1117] rounded-xl border border-gray-200 dark:border-white/10 p-6 shadow-sm">
                  <p className="text-lg font-serif text-gray-800 dark:text-gray-200 leading-relaxed mb-4">
                    {selectedEntry.definitionPt}
                  </p>
                  <p className="text-sm text-gray-400 dark:text-white/20 italic font-serif">
                ...
                <div className="bg-emerald-50/30 dark:bg-emerald-900/5 rounded-xl border border-emerald-100 dark:border-emerald-900/20 p-6 relative overflow-hidden group">
                  {loadingLexical ? (
                    ...
                    <button
                      ...
                      className="w-full py-4 rounded-lg bg-white dark:bg-white/5 border border-emerald-200 dark:border-emerald-900/30 text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] hover:bg-emerald-500 hover:text-white transition-all"
                    >
                      Ativar Análise Profunda
                    </button>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white dark:bg-[#0D1117] rounded-xl border border-gray-200 dark:border-white/10 p-4 shadow-sm flex flex-wrap gap-2">
                    {selectedEntry.relatedWords.map((w) => (
                      <span
                        key={w}
                        className="px-2 py-1 rounded bg-purple-50 dark:bg-purple-900/10 text-purple-600 text-[10px] font-bold border border-purple-100 dark:border-purple-900/20"
                      >
                        {w}
                      </span>
                    ))}
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(query.trim() ? results : allEntries.slice(0, 50)).map(
                  (entry) => (
                    <button
                      key={entry.number}
                      onClick={() => setSelectedEntry(entry)}
                      className="bg-white dark:bg-[#0D1117] rounded-xl border border-gray-200 dark:border-white/10 p-5 shadow-sm hover:border-indigo-500/30 transition-all text-left group"
                    >
                      <div className="flex justify-between items-start mb-3">
                        <span className="text-2xl font-serif text-gray-900 dark:text-white group-hover:text-indigo-600 transition-colors">
                          {entry.lemma}
                        </span>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-gray-500 dark:text-white/40 line-clamp-2 leading-relaxed">
                          {entry.definitionPt}
                        </p>
                      </div>
                    </button>
      ...
      <div className="p-4 bg-white dark:bg-[#0D1117] border-t border-gray-200 dark:border-white/10 flex items-center justify-center">
        <p className="text-[9px] text-gray-400 font-bold uppercase tracking-[0.3em]">
          Lexicon Integrado · Sola Scriptura
        </p>
      </div>
    </div>
  );
