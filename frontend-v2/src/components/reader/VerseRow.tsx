...
export const VerseRow: React.FC<VerseRowProps> = ({
  ...
  const highlightStyles = highlightColor
    ? THEOLOGICAL_COLORS[highlightColor]
    : null;
  ...
  return (
    <div
      className={`grid ${
        secondaryText ? "grid-cols-2 gap-12" : "grid-cols-1"
      } w-full max-w-4xl mx-auto`}
    >
      {/* Primary Version - Academic Book Page Style */}
      <div
        onClick={onClick}
        className={`group relative transition-all duration-300 py-4 px-8 rounded-lg cursor-pointer ${bgClass}`}
      >
          ...
          <div className="flex flex-col items-center pt-2 min-w-[24px]">
            ...
            {hasNote && (
              <button
                ...
                title="Ver/Editar anotação pessoal"
                className="mt-1 flex items-center justify-center w-5 h-5 rounded-md bg-amber-500/15 text-amber-600 dark:text-amber-400 hover:bg-amber-500/25 transition-colors"
              >
