...
export default function ResourceGuide() {
  const { activeVerseId, visibleVerseId, activeBook, activeChapter } =
    useTheoStore();
  ...
  const commentaries = useMemo(
    () => getCommentariesForReference(currentRef),
    [currentRef],
  );
  const crossRefs = useMemo(
    () => findRelatedReferences(currentRef),
    [currentRef],
  );
    ...
    <div className="flex flex-col h-full bg-background/40 backdrop-blur-3xl border-l border-border-subtle text-white overflow-hidden shadow-2xl">
      ...
      <div className="flex border-b border-border-subtle bg-background/60 p-1">
        {tabs.map((tab) => (
          ...
          >
            <tab.icon
              className={`w-4 h-4 mb-1 ${activeTab === tab.id ? "text-blue-400" : ""}`}
            />
          ...
          >
            ...
            <div className="relative p-4 rounded-2xl bg-gradient-to-br from-blue-500/10 to-purple-500/10 border border-blue-500/20 overflow-hidden group">
              ...
              <p className="text-xs text-white/70 leading-relaxed font-serif italic">
                {visibleVerseId
                  ? `Gerando análise profunda para ${visibleVerseId}...`
                  : "Selecione um versículo para obter insights exegéticos PhD via TheoAI."}
              </p>
            </div>
            ...
            <section>
              ...
              <div className="space-y-2">
                {commentaries.map((c) => (
                  <div
                    key={c.id}
                    className="p-3 rounded-xl bg-white/[0.02] border border-white/[0.05] hover:bg-white/[0.04] transition-all cursor-pointer group"
                  >
                    <div className="flex justify-between items-start mb-1.5">
                      <span className="text-[11px] font-bold text-white/80 group-hover:text-emerald-400 transition-colors">
                        {c.author}
                      </span>
                      <span className="text-[8px] px-1.5 py-0.5 rounded bg-white/5 text-white/30 uppercase">
                        {c.tradition}
                      </span>
                    </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-2">
                {crossRefs.map((ref, i) => (
                  <button
                    key={i}
                    className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02] border border-white/[0.05] hover:border-amber-500/30 text-left transition-all"
                  >
            ...
            <section className="pt-2">
              ...
              <div className="space-y-2">
                {relatedPeople.map((p) => (
                  <div
                    key={p.id}
                    className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.01] border border-white/[0.03] hover:bg-white/[0.03] transition-all cursor-pointer"
                  >
      ...
      <div className="px-6 py-4 border-t border-border-subtle bg-background/80">
        <p className="text-[8px] text-white/10 text-center uppercase tracking-[0.2em] font-black">
          TheoSphere Ultimate Suite
        </p>
      </div>
    </div>
  );
