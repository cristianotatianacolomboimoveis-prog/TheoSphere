...
function generateQuestions(passage: string): string[] {
  ...
  if (
    ...
    lower.includes("corinthians") ||
    lower.includes("galatians") ||
    lower.includes("ephesians") ||
    lower.includes("philippians") ||
    lower.includes("colossians")
  ) {
...
export default function StudyBuilder({ onClose }: { onClose: () => void }) {
  ...
  const copyStudyLink = () => {
    ...
    const base64 = btoa(
      unescape(encodeURIComponent(JSON.stringify(studyData))),
    );
                    ...
                    .map((study) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="text-[13px] font-semibold text-white/80 group-hover:text-cyan-400 transition-colors">
                              {study.title}
                            </h4>
                            <p className="text-[10px] text-white/25 mt-0.5">
                              {study.passage} · {study.questions.length}{" "}
                              perguntas
                            </p>
                          </div>
                          <button
              ...
              <div className="space-y-3">
                ...
                <div className="glass-amber rounded-xl p-4">
                  ...
                  <p className="text-[11px] text-white/40 leading-relaxed">
                    O TheoSphere irá gerar automaticamente perguntas de
                    discussão contextualizadas para a passagem escolhida (AT,
                    Evangelhos, Epístolas, etc.)
                  </p>
                </div>
                <button
                  onClick={createStudy}
                ...
                <div className="space-y-2">
                  {questions.map((q, i) => (
                    <div key={i} className="flex gap-2 items-start">
                      <span className="text-xs font-bold text-cyan-500/50 mt-2.5 w-5 text-right flex-shrink-0">
                        {i + 1}.
                      </span>
                      <input
                        type="text"
