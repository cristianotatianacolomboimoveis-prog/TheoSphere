...
export default function ManuscriptViewer({ reference }: { reference: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-background/40 text-white p-6 overflow-hidden">
      <div className="mb-6">
        <h2 className="text-sm font-black tracking-widest text-white/90 uppercase">
          Códex Virtual
        </h2>
        <p className="text-[10px] text-blue-400 font-bold tracking-[0.1em] uppercase">
          Visualização de Manuscritos Originais
        </p>
      </div>
      ...
      {/* Viewer Area */}
      <div className="flex-grow relative bg-black/40 border border-border-subtle rounded-3xl overflow-hidden group">
        ...
        {/* Floating Controls */}
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 p-2 bg-white/5 backdrop-blur-xl border border-border-strong rounded-2xl opacity-0 group-hover:opacity-100 transition-all duration-500">
          <button
      ...
      {/* Metadata */}
      <div className="mt-6 p-4 rounded-2xl bg-white/[0.02] border border-border-subtle">
        <div className="flex justify-between items-center mb-2">
          <span className="text-[10px] font-bold text-white/60 uppercase">
            {selectedCodex.name}
          </span>
          <span className="text-[9px] px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 font-bold uppercase">
            {selectedCodex.century} Century
          </span>
        </div>
        <p className="text-[11px] text-white/30 leading-relaxed italic">
          Este manuscrito é fundamental para a crítica textual. A visualização
          HD permite analisar variantes e glosas marginais em{" "}
          {selectedCodex.language}.
        </p>
      </div>
