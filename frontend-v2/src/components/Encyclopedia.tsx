...
export default function Encyclopedia({ onClose }: { onClose: () => void }) {
  ...
  const TABS: {
    ...
    },
    {
      ...
      label: "Eventos",
      icon: Calendar,
      count: BIBLICAL_EVENTS.length,
    },
    {
      id: "topics",
      label: "Tópicos",
      icon: Lightbulb,
      count: THEOLOGICAL_TOPICS.length,
    },
  ];
  ...
  const results = useMemo(() => {
    ...
    switch (activeTab) {
      case "people":
        base = BIBLICAL_PEOPLE;
        break;
      case "places":
        base = SEED_LOCATIONS;
        break;
      case "events":
        base = BIBLICAL_EVENTS;
        break;
      case "topics":
        base = THEOLOGICAL_TOPICS;
        break;
    }
    if (!q) return base;
          ...
          {selected ? (
            ...
            >
              <div className="text-center">
                <h3 className="text-4xl font-serif font-bold text-gray-900 dark:text-white mb-2">
                  {(selected.data as any).namePt ||
                    (selected.data as any).names?.pt}
                </h3>
                <div className="flex justify-center gap-2 mt-4">
                  <span className="px-3 py-1 rounded-full bg-orange-100 dark:bg-orange-900/20 text-orange-600 text-[10px] font-black uppercase tracking-widest">
                    {selected.type}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-[#0D1117] rounded-xl border border-gray-200 dark:border-white/10 p-8 shadow-sm">
                <p className="text-lg font-serif text-gray-800 dark:text-gray-200 leading-relaxed text-justify">
                  {(selected.data as any).description ||
                    (selected.data as any).theologicalSignificance}
                </p>
              </div>

              {selected.type === "places" &&
                (selected.data as any).coordinates && (
                  <div className="bg-white dark:bg-[#0D1117] rounded-xl border border-gray-200 dark:border-white/10 p-6 shadow-sm space-y-4">
                    ...
                    <p className="text-xs text-gray-500 dark:text-white/40 leading-relaxed">
                      Explore a arqueologia local e veja imagens reais de
                      satélite e 3D de como a região se encontra{" "}
                      <strong>hoje em dia</strong>.
                    </p>
                    <a
              ...
              {!aiAnalysis && !loadingAi ? (
                <button
                  onClick={() => generateAiAnalysis(selected.data)}
                  className="w-full py-4 rounded-xl bg-orange-600 text-white text-[11px] font-black uppercase tracking-[0.2em] shadow-lg shadow-orange-500/20 hover:scale-[1.02] transition-all"
                >
                  Aprofundar via IA
                </button>
              ) : (
            ...
            >
              {results.map((item, i) => (
                ...
                >
                  <div>
                    <span className="text-lg font-serif font-bold text-gray-900 dark:text-white group-hover:text-orange-600 transition-colors">
                      {item.namePt || item.names?.pt}
                    </span>
