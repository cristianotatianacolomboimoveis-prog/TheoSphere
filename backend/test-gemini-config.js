...
async function test(name, args) {
    ...
    try {
        ...
        console.log(`[${name}] Success in ${Date.now() - start}ms! Preview: ${result.text?.substring(0, 100).replace(/\n/g, ' ')}`);
    }
    catch (err) {
        console.error(`[${name}] Failed in ${Date.now() - start}ms:`, err);
    }
}
async function run() {
    await test('Text + systemInstruction', {
        model: 'gemini-2.5-flash',
        contents: 'Apresente os argumentos bíblicos e históricos para batismo por aspersão e por imersão.',
        config: {
            temperature: 0.7,
...
run();
//# sourceMappingURL=test-gemini-config.js.map