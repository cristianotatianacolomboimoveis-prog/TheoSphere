...
describe('McpController', () => {
  ...
  it('routes durable task polling and cancellation to the task store', async () => {
    const protocolTasks = {
    ...
    const update = await controller.handle({ jsonrpc: '2.0', id: 2, method: 'tasks/update', params: { taskId: 'task-123', inputResponses: { approval: { action: 'accept' } }, ...taskMeta } }, undefined, undefined, 'application/json', '2026-07-28', 'tasks/update', 'task-123', response);
    expect(update).toEqual({ jsonrpc: '2.0', id: 2, result: { resultType: 'complete' } });
    expect(protocolTasks.update).toHaveBeenCalledWith('task-123', { approval: { action: 'accept' } });
    const cancel = await controller.handle({ jsonrpc: '2.0', id: 3, method: 'tasks/cancel', params: { taskId: 'task-123', ...taskMeta } }, undefined, undefined, 'application/json', '2026-07-28', 'tasks/cancel', 'task-123', response);
    expect(cancel).toEqual({ jsonrpc: '2.0', id: 3, result: { resultType: 'complete' } });
