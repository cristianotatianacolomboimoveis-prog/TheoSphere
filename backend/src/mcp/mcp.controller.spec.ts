...
describe('McpController', () => {
  ...
  it('rejects session ids on modern stateless requests', async () => {
    const controller = new McpController(protocol, { get: jest.fn((key: string) => key === 'NODE_ENV' ? 'development' : undefined) } as unknown as ConfigService);
    await expect(controller.handle({ jsonrpc: '2.0', id: 1, method: 'ping', params: modernMeta }, undefined, 'legacy-session', 'application/json', '2026-07-28', 'ping', undefined, response)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('routes tasks by taskId in the modern Mcp-Name header', async () => {
    const controller = new McpController(protocol, { get: jest.fn((key: string) => key === 'NODE_ENV' ? 'development' : undefined) } as unknown as ConfigService);
    await expect(controller.handle({ jsonrpc: '2.0', id: 1, method: 'tasks/get', params: { taskId: 'task-123', ...modernMeta } }, undefined, undefined, 'application/json', '2026-07-28', 'tasks/get', 'task-123', response)).resolves.toBeDefined();
    await expect(controller.handle({ jsonrpc: '2.0', id: 2, method: 'tasks/get', params: { taskId: 'task-123', ...modernMeta } }, undefined, undefined, 'application/json', '2026-07-28', 'tasks/get', 'wrong-task', response)).rejects.toBeInstanceOf(BadRequestException);
  });
