...
describe('McpController', () => {
  ...
  it('requires the configured bearer key', async () => {
    ...
    await expect(controller.handle({}, undefined, undefined, 'application/json', undefined, undefined, undefined, response)).rejects.toBeInstanceOf(UnauthorizedException);
    await expect(controller.handle({}, 'Bearer ' + 'x'.repeat(32), undefined, 'application/json', undefined, undefined, undefined, response)).resolves.toBeDefined();
  });

  it('allows development without a configured key', async () => {
    const controller = new McpController(protocol, config);
    await expect(controller.handle({ jsonrpc: '2.0', id: 1, method: 'ping' }, undefined, undefined, 'application/json', undefined, undefined, undefined, response)).resolves.toBeDefined();
  });

  it('rejects malformed bodies and JSON-RPC batches', async () => {
    const controller = new McpController(protocol, config);
    await expect(controller.handle('bad', undefined, undefined, 'application/json', undefined, undefined, undefined, response)).rejects.toBeInstanceOf(BadRequestException);
    await expect(controller.handle([], undefined, undefined, 'application/json', undefined, undefined, undefined, response)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('returns 202 for a notification-only request', async () => {
    ...
    await expect(controller.handle({ jsonrpc: '2.0', method: 'notifications/initialized' }, undefined, undefined, 'application/json', undefined, undefined, undefined, response)).resolves.toBeUndefined();
