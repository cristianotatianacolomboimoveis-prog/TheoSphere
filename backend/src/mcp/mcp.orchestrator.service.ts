...
export class McpOrchestratorService {
  ...
  registerAgent(agent: McpAgent): McpAgent {
    ...
    if (registered.capabilities.includes('verification'))
      permissions.add('audit:read');
    for (const permission of permissions)
      this.security.grant(registered.id, permission as any);
    return registered;
  }
  ...
  assign(taskId: string, capability?: string): McpTask {
    ...
    if (
      task.files.length > 0 &&
      !task.requiredCapabilities.includes('coding')
    ) {
      throw new ConflictException(
        'MCP file-mutating tasks must require the coding capability',
      );
    }
    ...
    if (candidates.length === 0)
      throw new ConflictException(
        'No enabled MCP agent can execute task ' + taskId,
      );
    const compatible = candidates.find((agent) =>
    ...
    if (!compatible)
      throw new ConflictException(
        'No MCP agent satisfies all required capabilities for ' + taskId,
      );
  ...
  async lockAndStart(taskId: string): Promise<McpTask> {
    ...
    if (!task.assignedAgent)
      throw new ConflictException('MCP task must be assigned before locking');
    try {
      ...
      try {
        await this.locks.release(taskId, task.assignedAgent);
      } catch {
  ...
  async renewLocks(
    ...
    if (task.assignedAgent !== agentId)
      throw new ConflictException(
        `MCP lock renewal agent mismatch for task ${taskId}`,
      );
    if (
      !['LOCKED', 'IN_PROGRESS', 'IMPLEMENTED', 'TESTING', 'AUDITING'].includes(
        task.status,
      )
    ) {
      throw new ConflictException(
        `Task ${taskId} cannot renew locks from ${task.status}`,
      );
    }
    ...
    return {
      taskId,
      renewed,
      ttlMs: Number(process.env.MCP_LOCK_TTL_MS ?? 7_200_000),
    };
  }
  ...
  async advance(
    ...
    next: Extract<
      McpTaskState,
      'IMPLEMENTED' | 'TESTING' | 'AUDITING' | 'VERIFIED' | 'REWORK' | 'FAILED'
    >,
  ): Promise<McpTask> {
    const task = this.tasks.get(taskId);
    ...
    if (['VERIFIED', 'FAILED', 'REWORK'].includes(updated.status)) {
      if (task.assignedAgent) {
        try {
          ...
          /* task lifecycle remains authoritative */
        }
      }
    }
    return updated;
