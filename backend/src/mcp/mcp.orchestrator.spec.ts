...
describe('MCP orchestrator', () => {
  ...
  it('plans, routes, locks and starts a coding task', async () => {
    ...
    const task = tasks.create({
      ...
      description: 'Implement MCP code',
      priority: 'HIGH',
      files: ['src/a.ts'],
      dependencies: [],
    ...
    await expect(orchestrator.lockAndStart(task.id)).resolves.toEqual(
  ...
  it('does not start a task with an unresolved dependency', () => {
    ...
    const task = tasks.create({
      title: 'Dependent',
      description: 'Code after dependency',
      priority: 'HIGH',
      files: [],
      dependencies: [dep.id],
      requiredCapabilities: ['coding'],
    ...
    expect(() => tasks.transition(task.id, 'LOCKED')).toThrow(
