...
export class McpAutonomyService {
  ...
  async dispatch(taskId: string): Promise<McpTask> {
    ...
    if (planned.status !== 'PLANNED')
      throw new ConflictException(
        `Task ${taskId} is not dispatchable from ${planned.status}`,
      );
  ...
  async renewLocks(
    taskId: string,
    agentId: string,
  ): Promise<{ taskId: string; renewed: number; ttlMs: number }> {
  ...
  async recordResult(
    ...
    if (
      !['IN_PROGRESS', 'IMPLEMENTED', 'TESTING', 'AUDITING'].includes(
        task.status,
      )
    ) {
      throw new ConflictException(
        `Task ${taskId} cannot record a result from ${task.status}`,
      );
    }
    ...
    let updated = task;
    if (success && receipt)
      updated = this.tasks.recordExecutionReceipt(taskId, agentId, receipt);
    if (task.status === 'IN_PROGRESS')
      updated = await this.orchestrator.advance(
        taskId,
        success ? 'IMPLEMENTED' : 'REWORK',
      );
    if (success && updated.status === 'IMPLEMENTED')
      updated = await this.orchestrator.advance(taskId, 'TESTING');
    if (success && updated.status === 'TESTING')
      updated = await this.orchestrator.advance(taskId, 'AUDITING');
    if (!success && updated.status !== 'REWORK')
      updated = await this.orchestrator.advance(taskId, 'REWORK');
  ...
  private assertValidReceipt(
    receipt?: McpExecutionReceipt,
  ): asserts receipt is McpExecutionReceipt {
    if (
      !receipt ||
      typeof receipt.commitSha !== 'string' ||
      !receipt.commitSha.trim()
    ) {
      throw new ConflictException(
        'Successful MCP result requires a non-empty commitSha',
      );
    }
    if (
      !Array.isArray(receipt.changedFiles) ||
      receipt.changedFiles.some((path) => typeof path !== 'string')
    ) {
      throw new ConflictException(
        'Execution receipt changedFiles must be an array of strings',
      );
    }
    if (!Array.isArray(receipt.tests) || receipt.tests.length === 0) {
      throw new ConflictException(
        'Successful MCP result requires at least one test result',
      );
    }
    ...
    for (const test of receipt.tests) {
      if (!test || typeof test.command !== 'string' || !test.command.trim()) {
        throw new ConflictException(
          'Execution receipt test commands must be non-empty',
        );
      }
      ...
      if (
        ...
        (!Number.isFinite(test.durationMs) || test.durationMs < 0)
      ) {
        throw new ConflictException(
          'Execution receipt test durationMs is invalid',
        );
      }
    }
    if (receipt.tests.some((test) => test.status !== 'passed')) {
      throw new ConflictException(
        'Successful MCP result requires all reported tests to pass',
      );
    }
    if (
      receipt.artifactRefs !== undefined &&
      (!Array.isArray(receipt.artifactRefs) ||
        receipt.artifactRefs.some((ref) => typeof ref !== 'string'))
    ) {
      throw new ConflictException(
        'Execution receipt artifactRefs must be an array of strings',
      );
    }
    if (
      receipt.agentVersion !== undefined &&
      typeof receipt.agentVersion !== 'string'
    ) {
      throw new ConflictException(
  ...
  async verifyResult(
    ...
    if (task.status !== 'AUDITING') {
      throw new ConflictException(
        `Task ${taskId} cannot be verified from ${task.status}`,
      );
    }
    if (!task.executionReceipt)
      throw new ConflictException(
        'MCP verification requires a structured execution receipt',
      );
    if (!task.assignedAgent || task.assignedAgent === verifierAgentId) {
      throw new ConflictException(
        'MCP verification requires an independent agent',
      );
    }
    ...
    if (
      !verifier ||
      !verifier.enabled ||
      !verifier.capabilities.includes('verification')
    ) {
      throw new ConflictException(
        'MCP verifier must be a registered enabled agent with verification capability',
      );
    }
