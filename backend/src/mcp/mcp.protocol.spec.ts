...
describe('McpProtocolService', () => {
  ...
  const theology = {
    research: jest.fn(async () => ({ version: 1, items: [] })),
  } as any;
  const autonomy = {
    dispatch: jest.fn((id: string) => ({ id, status: 'IN_PROGRESS' })),
    recordResult: jest.fn(
      async (
        ...
        receipt: unknown,
      ) => ({ taskId: id, status: 'VERIFIED', receipt }),
    ),
  } as any;
  const memory = {
    search: jest.fn(async () => []),
    append: jest.fn(async (input: unknown) => ({
      id: 'MEM-1',
      ...(input as object),
    })),
  } as any;
  ...
  const rag = {
    chatWithEvidencePack: jest.fn(async (query: string) => ({
  ...
  it('acknowledges tasks/update while preserving protocol semantics', async () => {
    ...
    expect(protocolTasks.update).toHaveBeenCalledWith('task-1', {
  ...
  it('returns an async task handle for research calls from a tasks-capable client', async () => {
    ...
    theology.research.mockResolvedValueOnce({
      version: 1,
      query: 'grace',
      items: [],
    });
    ...
    expect(protocolTasks.complete).toHaveBeenCalledWith(
      'task-research-1',
      expect.objectContaining({
  ...
  it('supports MCP initialize and tool discovery', async () => {
    ...
    expect((listed?.result as any).tools).toEqual(
  ...
  it('routes Theo Engine research and preserves the EvidencePack contract', async () => {
    ...
    expect((response?.result as any).structuredContent).toEqual(
      expect.objectContaining({
        items: [expect.objectContaining({ reference: 'John 1:14' })],
      }),
    );
  });
  ...
  it('routes end-to-end Theo research into evidence-aware RAG', async () => {
    ...
    const response = await service.handle({
      ...
      id: 8,
      method: 'tools/call',
      params: { name: 'theosphere_answer', arguments: { query: 'grace' } },
    });
    expect(theology.research).toHaveBeenCalledWith('grace', 12);
    expect(rag.chatWithEvidencePack).toHaveBeenCalledWith(
      'grace',
      expect.anything(),
      undefined,
      undefined,
    );
    expect((response?.result as any).structuredContent.content).toBe(
      'answer:grace',
    );
  });
  ...
  it('routes autonomous execution tools', async () => {
    const response = await service.handle({
      ...
      params: {
        ...
        arguments: { taskId: 'TSK-1' },
      },
    });
    expect(autonomy.dispatch).toHaveBeenCalledWith('TSK-1');
    expect((response?.result as any).structuredContent.status).toBe(
      'IN_PROGRESS',
    );
  });
  ...
  it('enforces protocol permissions before tool execution', async () => {
    ...
    const denied = await service.handle({
      ...
      id: 10,
      method: 'tools/call',
      params: { name: 'theosphere_memory_search', arguments: { query: 'x' } },
    });
  ...
  it('passes a structured execution receipt to autonomy', async () => {
    ...
    expect((response?.result as any).structuredContent.receipt).toEqual(
      receipt,
    );
  });
  ...
  it('requires the assigned worker identity when recording results', async () => {
    const response = await service.handle({
      ...
      params: {
        name: 'theosphere_record_result',
        arguments: { taskId: 'TSK-1', success: true },
      },
    });
  ...
  it('rejects ping in modern protocol while retaining legacy compatibility', async () => {
    const modern = await service.handle(
  ...
  it('does not answer notifications', async () => {
    await expect(
      service.handle({ jsonrpc: '2.0', method: 'notifications/initialized' }),
    ).resolves.toBeNull();
  });
  ...
  it('routes independent verification separately from worker result recording', async () => {
    autonomy.verifyResult = jest.fn(async (id: string, verifierId: string) => ({
      taskId: id,
      status: 'VERIFIED',
    ...
    const response = await service.handle({
      ...
      params: {
        name: 'theosphere_verify_result',
        arguments: { taskId: 'TSK-1', verifierAgentId: 'verifier-1' },
      },
    });
    expect(autonomy.verifyResult).toHaveBeenCalledWith(
      'TSK-1',
      'verifier-1',
      undefined,
    );
