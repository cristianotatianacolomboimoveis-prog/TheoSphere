...
describe('MCP control-plane foundation', () => {
  ...
  it('uses default-deny permissions for unknown agents', () => {
    const security = new McpSecurityService();
    expect(() =>
      security.assertAllowed('unknown-agent', 'task:create'),
    ).toThrow('MCP permission denied');
  });
  ...
  it('normalizes agent capabilities and routes capable agents', () => {
    ...
    expect(registry.findCapable('coding').map((agent) => agent.id)).toEqual([
      'claude',
    ]);
  ...
  it('recovers the latest persisted agent snapshot without restoring stale duplicates', async () => {
    ...
    expect(registry.get('claude')).toEqual({
      ...agent,
      capabilities: ['coding'],
    });
  });
  ...
  it('recovers the complete persisted audit history instead of a fixed history window', async () => {
    ...
    const latestByKeyPrefix = jest.fn().mockResolvedValue([
      {
        content: JSON.stringify(newer),
        createdAt: new Date('2026-09-18T02:00:00.000Z'),
      },
      {
        content: JSON.stringify(older),
  ...
  it('persists and reloads a completed protocol task', async () => {
    ...
    const created = await first.create('theosphere_answer', {
      query: 'John 3:16',
    });
    await first.complete(created.taskId, {
      content: [{ type: 'text', text: 'Evidence-grounded answer' }],
      isError: false,
    });
  ...
  it('cancels a protocol task and rejects terminal completion', async () => {
    ...
    await expect(service.complete(task.taskId, {})).rejects.toThrow(
      'already terminal',
    );
  });
});
