...
export class McpController {
  ...
  async handle(
    ...
    const declaredProtocolVersion =
      ...
      typeof requestObject.params === 'object' &&
      !Array.isArray(requestObject.params)
        ? (requestObject.params as Record<string, unknown>)._meta &&
          typeof (requestObject.params as Record<string, unknown>)._meta ===
            'object' &&
          !Array.isArray(
            (requestObject.params as Record<string, unknown>)._meta,
          )
          ? (
              (requestObject.params as Record<string, unknown>)._meta as Record<
                string,
                unknown
              >
    ...
    if (
      requestedVersion &&
      !this.protocol.supportedProtocolVersions.includes(requestedVersion)
    ) {
    ...
    if (
      declaredProtocolVersion === this.protocol.protocolVersion &&
      requestedVersion !== this.protocol.protocolVersion
    ) {
      return this.protocolError(
        res,
        requestId,
    ...
    const modernNotification =
      typeof modernMethod === 'string' &&
      modernMethod.startsWith('notifications/');
    if (modern && !modernNotification) {
      if (sessionId)
        return this.protocolError(
          ...
          requestId,
          -32020,
          'MCP-Session-Id must not be sent for MCP 2026-07-28',
        );
      if (mcpMethod !== modernMethod)
        return this.protocolError(
          res,
          requestId,
          -32020,
          'Mcp-Method header must match JSON-RPC method',
        );
      if (modernMethod === 'tools/call') {
        const toolName =
          ...
          typeof requestObject.params === 'object' &&
          !Array.isArray(requestObject.params)
            ? (requestObject.params as Record<string, unknown>).name
            : undefined;
        if (mcpName !== toolName)
          return this.protocolError(
            res,
            requestId,
            -32020,
            'Mcp-Name header must match tools/call name',
          );
      } else if (
        modernMethod === 'tasks/get' ||
        modernMethod === 'tasks/update' ||
        modernMethod === 'tasks/cancel'
      ) {
        const taskId =
          requestObject?.params &&
          typeof requestObject.params === 'object' &&
          !Array.isArray(requestObject.params)
            ? (requestObject.params as Record<string, unknown>).taskId
            : undefined;
        if (mcpName !== taskId)
          return this.protocolError(
            res,
            requestId,
            -32020,
            'Mcp-Name header must match taskId',
          );
      }
    }
    if (Array.isArray(body))
      throw new BadRequestException(
        'MCP JSON-RPC batching is not supported by protocol 2025-06-18+',
      );
    if (!body || typeof body !== 'object')
      throw new BadRequestException(
        'MCP request body must be a JSON-RPC object',
      );
    ...
    if (modern) {
      if (
        ...
        return this.protocolError(
          ...
          requestId,
          -32602,
          'MCP protocol version metadata is required and must match 2026-07-28',
        );
    ...
    if (modern) effectiveProtocolVersion = this.protocol.protocolVersion;
      ...
      effectiveProtocolVersion =
        requested && ['2025-11-25', '2025-06-18'].includes(requested)
          ? requested
      ...
      if (!sessionVersion)
        throw new UnauthorizedException('Unknown MCP-Session-Id');
    ...
    const response = await this.protocol.handle(body, effectiveProtocolVersion);
    if (
      modern &&
  ...
  handleGet(
    ...
    if (requestedVersion === this.protocol.protocolVersion)
      throw new MethodNotAllowedException(
        'MCP 2026-07-28 is stateless; GET stream is unavailable',
      );
    const sessionVersion = sessionId ? this.sessions.get(sessionId) : undefined;
    if (!sessionId || !sessionVersion)
      throw new BadRequestException(
        'MCP-Session-Id is required for the Streamable HTTP GET stream',
      );
    if (requestedVersion && requestedVersion !== sessionVersion)
      throw new BadRequestException(
        'MCP protocol version does not match the session',
  ...
  handleDelete(
    ...
    if (requestedVersion === this.protocol.protocolVersion)
      throw new MethodNotAllowedException(
        'MCP 2026-07-28 is stateless; DELETE session is unavailable',
      );
    const sessionVersion = sessionId ? this.sessions.get(sessionId) : undefined;
    if (!sessionId || !sessionVersion)
      throw new BadRequestException(
        'MCP-Session-Id is required for session termination',
      );
    if (requestedVersion && requestedVersion !== sessionVersion)
      throw new BadRequestException(
        'MCP protocol version does not match the session',
  ...
  private validateAccept(accept?: string): void {
    ...
    if (
      ...
      throw new BadRequestException(
        'MCP Accept must include application/json or text/event-stream',
      );
  }
  ...
  private authorize(authorization?: string): void {
    ...
    if (!configured) {
      if (production)
        throw new UnauthorizedException(
          'MCP endpoint is disabled until MCP_API_KEY is configured',
        );
      return;
    }
    ...
    if (
      ...
      throw new UnauthorizedException('Invalid MCP authorization');
  }
}
