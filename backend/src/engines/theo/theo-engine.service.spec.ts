...
describe('TheologyEngineService', () => {
  it('builds a deterministic evidence pack from hybrid Bible hits and cross-references', async () => {
    ...
    const search = {
      hybridSearchVerses: jest.fn(async () => [
        {
          id: 'v1',
          bookId: 43,
          chapter: 3,
    ...
    const crossReferences = {
      list: jest.fn(async () => [{ target: 'Romans 5:8', rank: 1, votes: 50 }]),
    } as any;
    const linguistics = {
      getInterlinearChapter: jest.fn(async () => ({ verses: {} })),
    } as any;
    const evidencePacks = {
      build: jest.fn((query: string, inputs: any[], limit: number) => ({
        query,
    ...
    const service = new TheologyEngineService(
      prisma,
      search,
      evidencePacks,
      crossReferences,
  ...
  it('clamps research limits and handles empty queries', async () => {
    const evidencePacks = {
      build: jest.fn((query: string, inputs: any[]) => ({ query, inputs })),
    } as any;
    const search = { hybridSearchVerses: jest.fn(async () => []) } as any;
    const prisma = {
      book: { findMany: jest.fn(async () => []) },
      technicalCommentary: { findMany: jest.fn(async () => []) },
    } as any;
    ...
    expect(search.hybridSearchVerses).toHaveBeenCalledWith('tema', {
      limit: 50,
    });
  });
});
