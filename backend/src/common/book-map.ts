...
export const BOOK_ID_TO_NAME_EN: Record<number, string> = {
  1: 'Genesis',
  2: 'Exodus',
  3: 'Leviticus',
  4: 'Numbers',
...
export function normalizeRefToCanonicalEn(ref: string): string {
  ...
  const m = trimmed.match(/^(\d?\s*[A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s]*?)\s+(\d+):(\d+)$/);
  if (!m) return trimmed;
  const bookKey = m[1].toLowerCase().replace(/\s+/g, ' ').trim();
  const bookId = BOOK_NAME_TO_ID[bookKey];
  if (!bookId || !BOOK_ID_TO_NAME_EN[bookId]) return trimmed;
  return `${BOOK_ID_TO_NAME_EN[bookId]} ${m[2]}:${m[3]}`;
}
