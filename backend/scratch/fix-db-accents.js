...
async function main() {
  console.log('Iniciando atualização de acentuação no banco de dados...');
  
  for (const [id, name] of Object.entries(bookMap)) {
    const result = await prisma.bibleVerse.updateMany({
      where: { bookId: parseInt(id) },
      data: { book: name }
    });
    if (result.count > 0) {
      console.log(`Atualizado: ${name} (${result.count} versículos)`);
    }
  }
