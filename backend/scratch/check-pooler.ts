...
async function run() {
  // 1. Direct on 5432
  const urlDirect =
    'postgresql://postgres:REDACTED@db.chjywahtwktqqxqlthvc.supabase.co:5432/postgres';
  await check(urlDirect, 'Direct 5432');

  // 2. Pooler on 6543
  const urlPooler =
    'postgresql://postgres:REDACTED@db.chjywahtwktqqxqlthvc.supabase.co:6543/postgres';
  await check(urlPooler, 'Pooler 6543');

  // 3. Pooler on 6543 with transaction mode (postgres.[project-ref])
  const urlPoolerMode =
    'postgresql://postgres.chjywahtwktqqxqlthvc:REDACTED@aws-0-sa-east-1.pooler.supabase.com:6543/postgres';
